...
$Scores_add = NULL; // Initialize page object first

class cScores_add extends cScores {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["ScoreID"] != "") {
				...
				$this->setKey("ScoreID", $this->ScoreID->CurrentValue); // Set up key
			} else {
				$this->setKey("ScoreID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "Scoresview.php")
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$this->ScoreID->setDbValue($conn->Insert_ID());
			$rsnew['ScoreID'] = $this->ScoreID->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($Scores_add)) $Scores_add = new cScores_add();

// Page init
$Scores_add->Page_Init();

// Page main
$Scores_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$Scores_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var Scores_add = new ew_Page("Scores_add");
Scores_add.PageID = "add"; // Page ID
var EW_PAGE_ID = Scores_add.PageID; // For backward compatibility

// Form object
var fScoresadd = new ew_Form("fScoresadd");

// Validate form
fScoresadd.Validate = function() {
...
fScoresadd.Form_CustomValidate = 
...
fScoresadd.ValidateRequired = true;
<?php } else { ?>
fScoresadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fScoresadd.Lists["x_ID"] = {"LinkField":"x_ID","Ajax":null,"AutoFill":false,"DisplayFields":["x__Email","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fScoresadd.Lists["x_TopicID"] = {"LinkField":"x_TopicID","Ajax":null,"AutoFill":false,"DisplayFields":["x_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $Scores_add->ShowPageHeader(); ?>
<?php
$Scores_add->ShowMessage();
?>
<form name="fScoresadd" id="fScoresadd" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="Scores">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewGrid"><tr><td>
<table id="tbl_Scoresadd" class="table table-bordered table-striped">
...
<script type="text/javascript">
fScoresadd.Lists["x_ID"].Options = <?php echo (is_array($Scores->ID->EditValue)) ? ew_ArrayToJson($Scores->ID->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fScoresadd.Lists["x_TopicID"].Options = <?php echo (is_array($Scores->TopicID->EditValue)) ? ew_ArrayToJson($Scores->TopicID->EditValue, 1) : "[]" ?>;
</script>
</span>
