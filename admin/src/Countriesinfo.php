...
class cCountries extends cTable {
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('CountryID', $rs))
				ew_AddFilter($where, ew_QuotedName('CountryID') . '=' . ew_QuotedValue($rs['CountryID'], $this->CountryID->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->CountryID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@CountryID@", ew_AdjustSql($this->CountryID->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->CountryID->CurrentValue)) {
			$sUrl .= "CountryID=" . urlencode($this->CountryID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["CountryID"]; // CountryID
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->CountryID->CurrentValue = $key;
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->CountryID->Exportable) $Doc->ExportCaption($this->CountryID);
				...
				if ($this->ContinentCode->Exportable) $Doc->ExportCaption($this->ContinentCode);
				if ($this->DisplayOrder->Exportable) $Doc->ExportCaption($this->DisplayOrder);
			} else {
				if ($this->CountryID->Exportable) $Doc->ExportCaption($this->CountryID);
				if ($this->Code->Exportable) $Doc->ExportCaption($this->Code);
				if ($this->Name->Exportable) $Doc->ExportCaption($this->Name);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->CountryID->Exportable) $Doc->ExportField($this->CountryID);
					...
					if ($this->ContinentCode->Exportable) $Doc->ExportField($this->ContinentCode);
					if ($this->DisplayOrder->Exportable) $Doc->ExportField($this->DisplayOrder);
				} else {
					if ($this->CountryID->Exportable) $Doc->ExportField($this->CountryID);
					if ($this->Code->Exportable) $Doc->ExportField($this->Code);
					if ($this->Name->Exportable) $Doc->ExportField($this->Name);
