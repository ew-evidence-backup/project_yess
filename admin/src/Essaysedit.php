...
$Essays_edit = NULL; // Initialize page object first

class cEssays_edit extends cEssays {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->EssayID->CurrentValue == "")
			$this->Page_Terminate("Essayslist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->EssayID->FldIsDetailKey)
			$this->EssayID->setFormValue($objForm->GetValue("x_EssayID"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->EssayID->CurrentValue = $this->EssayID->FormValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// EssayID
			$this->EssayID->EditCustomAttributes = "";
			$this->EssayID->EditValue = $this->EssayID->CurrentValue;
			$this->EssayID->ViewCustomAttributes = "";
...
// Create page object
if (!isset($Essays_edit)) $Essays_edit = new cEssays_edit();

// Page init
$Essays_edit->Page_Init();

// Page main
$Essays_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$Essays_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var Essays_edit = new ew_Page("Essays_edit");
Essays_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = Essays_edit.PageID; // For backward compatibility

// Form object
var fEssaysedit = new ew_Form("fEssaysedit");

// Validate form
fEssaysedit.Validate = function() {
...
fEssaysedit.Form_CustomValidate = 
...
fEssaysedit.ValidateRequired = true;
<?php } else { ?>
fEssaysedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fEssaysedit.Lists["x_TopicID"] = {"LinkField":"x_TopicID","Ajax":null,"AutoFill":false,"DisplayFields":["x_Name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $Essays_edit->ShowPageHeader(); ?>
<?php
$Essays_edit->ShowMessage();
?>
<form name="fEssaysedit" id="fEssaysedit" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="Essays">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewGrid"><tr><td>
<table id="tbl_Essaysedit" class="table table-bordered table-striped">
<?php if ($Essays->EssayID->Visible) { // EssayID ?>
	<tr id="r_EssayID">
		<td><span id="elh_Essays_EssayID"><?php echo $Essays->EssayID->FldCaption() ?></span></td>
		<td<?php echo $Essays->EssayID->CellAttributes() ?>>
<span id="el_Essays_EssayID" class="control-group">
<span<?php echo $Essays->EssayID->ViewAttributes() ?>>
<?php echo $Essays->EssayID->EditValue ?></span>
</span>
<input type="hidden" data-field="x_EssayID" name="x_EssayID" id="x_EssayID" value="<?php echo ew_HtmlEncode($Essays->EssayID->CurrentValue) ?>">
<?php echo $Essays->EssayID->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fEssaysedit.Lists["x_TopicID"].Options = <?php echo (is_array($Essays->TopicID->EditValue)) ? ew_ArrayToJson($Essays->TopicID->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<?php echo $Essays->DateModified->EditValue ?></span>
</span>
<input type="hidden" data-field="x_DateModified" name="x_DateModified" id="x_DateModified" value="<?php echo ew_HtmlEncode($Essays->DateModified->CurrentValue) ?>">
...
ew_CreateCalendar("fEssaysedit", "x_Date", "%Y/%m/%d");
</script>
<?php } ?>
</span>
