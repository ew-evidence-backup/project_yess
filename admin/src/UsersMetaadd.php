...
$UsersMeta_add = NULL; // Initialize page object first

class cUsersMeta_add extends cUsersMeta {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["MetaID"] != "") {
				...
				$this->setKey("MetaID", $this->MetaID->CurrentValue); // Set up key
			} else {
				$this->setKey("MetaID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "UsersMetaview.php")
	...
	function LoadDefaultValues() {
		...
		$this->EducationalStatus->CurrentValue = 0;
	...
	function AddRow($rsold = NULL) {
		...
		// EducationalStatus
		$this->EducationalStatus->SetDbValueDef($rsnew, $this->EducationalStatus->CurrentValue, 0, strval($this->EducationalStatus->CurrentValue) == "");
		...
		if ($AddRow) {
			$this->MetaID->setDbValue($conn->Insert_ID());
			$rsnew['MetaID'] = $this->MetaID->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($UsersMeta_add)) $UsersMeta_add = new cUsersMeta_add();

// Page init
$UsersMeta_add->Page_Init();

// Page main
$UsersMeta_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$UsersMeta_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var UsersMeta_add = new ew_Page("UsersMeta_add");
UsersMeta_add.PageID = "add"; // Page ID
var EW_PAGE_ID = UsersMeta_add.PageID; // For backward compatibility

// Form object
var fUsersMetaadd = new ew_Form("fUsersMetaadd");

// Validate form
fUsersMetaadd.Validate = function() {
...
fUsersMetaadd.Form_CustomValidate = 
...
fUsersMetaadd.ValidateRequired = true;
<?php } else { ?>
fUsersMetaadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fUsersMetaadd.Lists["x_ID"] = {"LinkField":"x_ID","Ajax":null,"AutoFill":false,"DisplayFields":["x__Email","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $UsersMeta_add->ShowPageHeader(); ?>
<?php
$UsersMeta_add->ShowMessage();
?>
<form name="fUsersMetaadd" id="fUsersMetaadd" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="UsersMeta">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewGrid"><tr><td>
<table id="tbl_UsersMetaadd" class="table table-bordered table-striped">
...
<script type="text/javascript">
fUsersMetaadd.Lists["x_ID"].Options = <?php echo (is_array($UsersMeta->ID->EditValue)) ? ew_ArrayToJson($UsersMeta->ID->EditValue, 1) : "[]" ?>;
</script>
</span>
